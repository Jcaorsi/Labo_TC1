...
\par Otro valor estudiado a lo largo del tp fue el tiempo de respuesta transitoria, el cual se define como 5$\tau$ en sistemas de primer orden.
En una de las configuraciones se observó que la respuesta fue subamortiguada con coeficientes de módulos muy dispares, es decir con una diferencia de orden mayor a uno, notándose en que uno de los términos de la respuesta pesó o influyó mucho más que el otro (puesto que decrece mas rápido); es por esto que se consideró que, en ese caso, se podía aproximar el sistema a uno de primer orden, y buscamos el $\tau$ acorde a esto. Así, tras un desarrollo de la ecuación diferencial obtenida, se llegó a que
\begin{equation*}
    V_c(t) = V_s (1-e^{st})
\end{equation*}
...
\par Con el fin de facilitar la comparación de los tiempos de respuesta para las distintas configuraciones, se tomó que el tiempo de respuesta era igual a $5\tau$ aun cuando no se podía realizar la aproximación a primer orden. En estos casos, se tomó como tiempo de respuesta $5\tau$ el instante en que la salida era igual a $V_s \pm 5\% $, y a partir de eso se despejó $\tau$.
...
\end{figure}
    ...
    \begin{enumerate}
        ...
        \item En esta parte, se ajustó la resistencia Rv a su valor máximo. Poteriormente, se coloco uno de los cursores horizontales del osciloscopio en $ V_{salida}=0$ V, y el otro en $ V_{salida}=3,16$ V y medimos el $ \Delta t $ entre esos valores, tomandolo como el $\tau$ del sistema.
    ...
    \subsection{Mediciones}
        ...
        \subsubsection*{Capacitor de C = 47 pF}
            \begin{itemize}
                ...
                \item Tiempo en que la salida llegó a 3,16 V con $ R_V = R_{V_{crítico}}: t = 2,20 \mu s$ % item 3
    ...
    \subsection{Cálculos}
        ...
        \subsubsection*{Ecuaciones utilizadas}            
            ...
            \begin{equation}
                L = \frac{C \cdot R_{eq}^2}{4} = \frac{C \cdot (R_v+R_f+R_L)^2}{4}
            \end{equation}
        ...
        \subsubsection*{Resultados}
            ...
            \subsubsection*{Capacitor de C = 470 pF}
                ...
                \begin{equation*}
                    L=\frac{(1,9\cdot10^3 + 215 + 0,8)^2 \cdot 470 \cdot 10^{-12}}{4}= 0,526 mH
                \end{equation*} \par
                
                Resistencia variable tal que el amortiguamiento fuera crítico:
                \begin{equation*}
                    R_{critico} = 1.899 k\Omega
                \end{equation*} \par

        
                Valor $ \tau $ con $ R_V = R_{V_{max}} = 9980 \Omega $:
                 \begin{equation*}
                    \tau = 4,74 \mu s RAROOOOO
                \end{equation*}
	...
	Es importante mencionar que en este trabajo, no se contó con un valor de referencia de la inductancia; esta debió ser aproximada mediante el valor de la resistencia del amortiguamiento crítico obtenida con el capacitor de 470 pF. Consecuentemente, los datos teóricos también pueden contener errores asociados.
